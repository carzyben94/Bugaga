...
logging.info(f"📁 Все файлы: {os.listdir('.')}")

# ЯВНАЯ ЗАГРУЗКА МОДУЛЯ
...
# Загружаем logger.py как модуль
logger_path = os.path.join(os.getcwd(), "logger.py")

if os.path.exists(logger_path):
    logging.info("✅ logger.py найден, загружаю...")
    try:
        logger = load_module_from_file(logger_path, "logger")
        LogStorage = logger.LogStorage
        format_logs_for_display = logger.format_logs_for_display
        format_logs_for_copy = logger.format_logs_for_copy
        get_logs_keyboard = logger.get_logs_keyboard
        log_command = logger.log_command
        logging.info("✅ logger.py загружен успешно!")
    except Exception as e:
        logging.error(f"❌ Ошибка загрузки logger.py: {e}")
        raise
else:
    logging.error("❌ logger.py НЕ НАЙДЕН!")
    logging.info("💡 Создаю logger.py...")
