...
# ---------- СИСТЕМНЫЙ ПРОМТ (ТОЛЬКО АНГЛИЙСКИЙ) ----------
SYSTEM_PROMPT = """Ты — AI-ассистент для анализа веб-страниц.

🔥 ВАЖНО: ВСЕГДА ИСПОЛЬЗУЙ АНГЛИЙСКИЕ НАЗВАНИЯ!

Когда пользователь просит кликнуть или найти элемент:
1. Определи, что он хочет найти
2. Если в контексте есть список элементов — найди похожий
3. Используй ТОЧНОЕ название из DOM (английское)

ПРИМЕРЫ:
Пользователь: "нажми Обзор"
→ Если в контексте есть "Explore" → {"action": "click", "target": "Explore"}
→ Если в контексте нет → {"action": "ask", "question": "я не вижу элемент 'Обзор'"}

Пользователь: "нажми Главная"
→ Если в контексте есть "Home" → {"action": "click", "target": "Home"}

Пользователь: "введи текст в поиск"
→ {"action": "type", "text": "текст", "field": "search"}
...
🔥 ФОРМАТ ОТВЕТА:
Используй АНГЛИЙСКИЕ названия элементов в ответах!"""
...
def translate_to_english(text):
    """Переводит русский текст в английский для поиска"""
...
SYNONYMS = {
    ...
    'explore': ['explore', 'обзор', 'explore', 'discover'],
...
def get_meaning(text):
    """Определяет смысл текста по словарю синонимов (на английском)"""
    text_lower = text.lower().strip()
...
def get_synonyms(meaning):
    """Возвращает все синонимы для данного смысла"""
    return SYNONYMS.get(meaning, [])
...
class Memory:
    ...
    def get_element_names(self):
        """Возвращает список названий элементов на странице (для AI)"""
        if not self.current_snapshot:
            return []
        
        elements = self.current_snapshot.get('elements', [])
        names = []
    ...
    def get_context_for_ai(self):
        ...
        # 🔥 ДОБАВЛЯЕМ СПИСОК ЭЛЕМЕНТОВ ДЛЯ AI
        element_names = self.get_element_names()
        if element_names:
            context.append("\nAvailable elements on page:")
            for name in element_names[:20]:
...
When user asks to click or find an element:
1. Determine what they want to find
2. If context has element list — find similar
3. Use EXACT element name from DOM (English)

EXAMPLES:
User: "нажми Обзор"
→ If context has "Explore" → {"action": "click", "target": "Explore"}

User: "нажми Главная"
→ If context has "Home" → {"action": "click", "target": "Home"}

User: "введи текст в поиск"
→ {"action": "type", "text": "text", "field": "search"}

AVAILABLE ACTIONS:
1. navigate — go to website
2. ask — ask question about page
3. click — click element (target in ENGLISH)
4. type — type text into field (field in ENGLISH)
5. submit — submit form
6. screenshot — take screenshot
7. back — go back
8. history — show history
9. clear — clear memory

RULES:
- ALWAYS use ENGLISH names
- If you don't see element in context — use "ask"
- For type: field is always in ENGLISH
- For click: target is always in ENGLISH
...
class BrowserCDP:
    ...
    async def get_all_interactive_elements(self):
        """Собирает ВСЕ интерактивные элементы на странице"""
        try:
            js = """
                (function() {
                    const result = [];
                    ...
                    function getSelector(el) {
                        ...
                        // 3. По aria-label (ТОЛЬКО АНГЛИЙСКИЙ)
    ...
    async def get_snapshot(self):
        try:
            ...
            self.snapshot = {
                ...
                "interactive": interactive_elements[:100]
            }
    ...
    async def click_element(self, target):
        try:
            ...
            file_logger.log(f"🔍 Ищем '{target}' → переведено: '{english_target}'", "INFO")
            ...
            js = f"""
                (function() {{
                    ...
                    const elements = document.querySelectorAll('button, a, input, div[role="button"], [role="link"], [role="menuitem"], [role="tab"], input[type="submit"]');
            ...
            result = await self.eval_js(js)
            if result:
                file_logger.log(f"✅ Кликнул по '{target}' (англ: {english_target})", "INFO")
                return True
            else:
    ...
    async def type_text(self, text, field):
        try:
            ...
            # 🔥 ПЕРЕВОДИМ field НА АНГЛИЙСКИЙ
            english_field = translate_to_english(field)
            file_logger.log(f"🔍 Ищем поле '{field}' → переведено: '{english_field}'", "INFO")
            ...
            if result:
                file_logger.log(f"✅ Ввел '{text}' в поле {english_field} + Enter", "INFO")
                return True
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• какие кнопки есть?\n"
        "• введи текст в поиск\n\n"
        "🌐 AI сам переводит на английский для поиска!\n"
        "🍪 Авторизация на X.com!\n"
        "🔍 Находит элементы по смыслу!"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower() in ['привет', 'здравствуй', 'hello', 'hi']:
        await update.message.reply_text(
            ...
            "Можешь сказать 'зайди на x.com' или 'нажми Обзор'"
        )
        return
        ...
        try:
            ...
            if interactive:
                ...
                for el in interactive[:30]:
                    ...
                    if text_el:
                        interactive_text.append(f"• {visible} [{el_type}] {text_el}")
                
                if interactive_text:
                    context_text = "Interactive elements:\n" + "\n".join(interactive_text)
                else:
                    context_text = "No interactive elements found"
            else:
                context_text = "No interactive elements found"
...
def main():
    print("="*50)
    print("🚀 ЗАПУСК БОТА (AI ПЕРЕВОД + АВТОПОИСК)")
    print("="*50)
    print(f"📌 Chrome путь: {CHROME_PATH}")
    print("🕵️ Маскировка: ВКЛЮЧЕНА")
    print("🍪 Куки: ВКЛЮЧЕНЫ (X.com)")
    print("🌐 AI перевод: ВКЛЮЧЕН (русский → английский)")
    print("🔍 Поиск по смыслу: ВКЛЮЧЕН")
    print("📊 Интерактивные элементы: ВКЛЮЧЕНЫ")
    print("="*50)
    ...
    print("   • какие кнопки есть?")
    print("🌐 AI сам переводит русский в английский для поиска!")
    app.run_polling()
