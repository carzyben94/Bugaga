...
# ---------- СЛОВАРИ СИНОНИМОВ ----------
SYNONYMS = {
    'поиск': ['поиск', 'search', 'найти', 'find', 'query', 'запрос', 'искать'],
    'вход': ['вход', 'войти', 'login', 'sign in', 'log in', 'войдите'],
    'регистрация': ['регистрация', 'зарегистрироваться', 'sign up', 'register'],
    'отправить': ['отправить', 'send', 'submit', 'послать'],
    'закрыть': ['закрыть', 'close', 'закрой'],
    'имя': ['имя', 'name', 'ваше имя'],
    'email': ['email', 'e-mail', 'почта'],
    'пароль': ['пароль', 'password', 'pass'],
    'телефон': ['телефон', 'phone', 'номер'],
    'сообщение': ['сообщение', 'message', 'комментарий'],
}
        ...
        if self.current_snapshot and self.current_snapshot.get('elements'):
            ...
            if elements:
                ...
                for el in elements[:20]:
                    ...
                    if text:
                        context.append(f"- {visible} [{el_type}] {text}")
    ...
    def get_history_text(self):
        ...
        for i, action in enumerate(self.history, 1):
            ...
            if action_type == 'snapshot':
                ...
                lines.append(f"{i}. ✏️ [{timestamp}] Ввод '{data.get('text', '')}'")
...
ВОПРОСЫ (ask) — пользователь СПРАШИВАЕТ:
• "какие кнопки?"
• "есть ли поле?"
• "что видишь?"
...
"введи привет в поиск" → {"action": "type", "text": "привет", "field": "поиск"}
"нажми на Войти" → {"action": "click", "target": "Войти"}
...
def ask_ai_for_answer(prompt, context=None, memory=None):
    try:
        ...
        if context:
            messages.append({"role": "user", "content": f"СТРУКТУРА:\n{context}"})
            ...
            js = f"""
                (function() {{
                    ...
                    for (let el of elements) {{
                        ...
                        const allText = text + ' ' + aria + ' ' + id + ' ' + cls;
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 Привет! Я бот с ИИ-пониманием и сбором интерактивных элементов.\n\n"
        "🗣️ Говори как хочешь:\n"
        "• зайди на x.com\n"
        "• какие кнопки есть?\n"
        "• какие ссылки?\n"
        ...
        "🔍 Собираю все интерактивные элементы!"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower() in ['привет', 'здравствуй', 'hello', 'hi']:
        await update.message.reply_text(
            ...
            "Можешь сказать 'зайди на x.com' или 'какие кнопки?'"
        )
        return
    ...
    if action == 'ask':
        ...
        try:
            ...
            if interactive:
                ...
                if interactive_text:
                    context_text = f"Интерактивные элементы:\n" + "\n".join(interactive_text)
                else:
                    context_text = "Интерактивных элементов не найдено"
            else:
                context_text = "Интерактивных элементов не найдено"
...
def main():
    print("="*50)
    print("🚀 ЗАПУСК БОТА (СБОР ВСЕХ ИНТЕРАКТИВНЫХ ЭЛЕМЕНТОВ)")
    print("="*50)
    print(f"📌 Chrome путь: {CHROME_PATH}")
    print("🕵️ Маскировка: ВКЛЮЧЕНА")
    print("🍪 Куки: ВКЛЮЧЕНЫ (X.com)")
    print("🔍 Сбор элементов: ВКЛЮЧЕН (кнопки, ссылки, поля, меню, чекбоксы, радио, вкладки)")
    ...
    print("   • какие ссылки?")
    print("   • какие поля ввода?")
    print("🔍 Собираю ВСЕ интерактивные элементы!")
    app.run_polling()
