...
async def parse_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if error_check and ("doesn't exist" in error_check.lower() or "not found" in error_check.lower()):
        ...
        profile_data = await page.evaluate('''
            () => {
                ...
                if (tweetsEl) {
                    result.tweets_count = tweetsEl.innerText || '0';
                }
                ...
                if (verifiedEl) {
                    ...
                    if (color.includes('#1D9BF0')) {
                        result.is_verified = true;
                    } else if (color.includes('#FBAF00')) {
                ...
                if (pinnedEl) {
                    ...
                    if (pinnedTweet) {
                        const textEl = pinnedTweet.querySelector('[data-testid="tweetText"]');
                        const timeEl = pinnedTweet.querySelector('time');
                        const likesEl = pinnedTweet.querySelector('[data-testid="like"]');
                        const retweetEl = pinnedTweet.querySelector('[data-testid="retweet"]');
