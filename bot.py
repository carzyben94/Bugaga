...
ORCA_BASE_URL = os.environ.get("ORCAROUTER_BASE_URL", "https://api.orcarouter.ai/v1")
ORCA_MODEL = os.environ.get("ORCAROUTER_MODEL", "qwen/qwen3.8-27b-free")

if not TELEGRAM_TOKEN:
    raise ValueError("TELEGRAM_BOT_TOKEN не задан!")
if not ORCA_API_KEY:
    raise ValueError("ORCAROUTER_API_KEY не задан!")
...
# ============================================================
# ORCA ROUTER LM (OpenAI-compatible)
# ============================================================

class OrcaRouterLM(dspy.LM):
    """
    DSPy LM provider для Orca Router AI.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.api_key = api_key or ORCA_API_KEY
        self.base_url = (base_url or ORCA_BASE_URL).rstrip("/")
        self.model = model or ORCA_MODEL
    ...
    def forward(self, prompt=None, messages=None, **kwargs):
        if not self.api_key:
            raise RuntimeError("ORCAROUTER_API_KEY не задан")
        ...
        # Поддержка function calling для DSPy ReAct
        if params.get("tools"):
        ...
        try:
            with httpx.Client(
                timeout=httpx.Timeout(
                    connect=30,
                    read=180,  # Больший таймаут для ReAct агента
                    write=30,
                    pool=30,
                )
            ) as client:
            ...
            logger.error(
                "Orca Router HTTP %s: %s",
                e.response.status_code,
                body,
            )
            raise RuntimeError(
                f"Orca Router HTTP {e.response.status_code}: {body}"
            ) from e

        except Exception as e:
            logger.exception("Orca Router API")
            raise RuntimeError(f"Orca Router API error: {e}") from e

        choices = data.get("choices") or []
        if not choices:
            raise RuntimeError(f"Orca Router вернул пустой ответ: {data}")

        message = choices[0].get("message") or {}
        content = message.get("content") or ""

        # Если модель вернула tool_calls, форматируем их для DSPy
        tool_calls = message.get("tool_calls")
        if tool_calls and not content:
            # Преобразуем tool_calls в текстовый формат для DSPy ReAct
            tc_texts = []
            for tc in tool_calls:
                func = tc.get("function", {})
                name = func.get("name", "unknown_tool")
                args = func.get("arguments", "{}")
                tc_texts.append(f"Tool call: {name}({args})")
            content = "\n".join(tc_texts)
...
def init_dspy():
    ...
    if not ORCA_API_KEY:
        logger.warning("ORCAROUTER_API_KEY не задан")
        return False
...
async def main():
    ...
    logger.info("Orca Router: %s", ORCA_BASE_URL)
    logger.info("Orca Model: %s", ORCA_MODEL)
    ...
    logger.info("DSPy (Orca Router): %s", "OK" if dspy_ok else "FAIL")
