...
async def get_browser_page():
    ...
    # Создаём контекст с полной эмуляцией
...
async def take_screenshot(url: str) -> bytes:
    ...
    try:
        ...
        # Прокручиваем для подгрузки ленивых изображений
        await page.evaluate("""
...
async def get_page_content(url: str) -> str:
    """Получает HTML-код страницы после полной загрузки"""
    page, browser, context = await get_browser_page()
...
async def execute_js(url: str, script: str):
    """Выполняет JS на странице и возвращает результат"""
    page, browser, context = await get_browser_page()
