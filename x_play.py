# x_play.py — X/Twitter агент через Playwright с авторизацией
import os
import time
import json
import re
...
X_EMAIL = os.environ.get("X_EMAIL")  # Иногда требуется для 2FA
...
class XPlayAgent:
    ...
    def _login(self, page):
        ...
        try:
            ...
            # Проверка на email/телефон (если спрашивает)
            try:
    ...
    def _ensure_browser(self, headless=True):
        ...
        try:
            ...
            has_cookies = self._load_cookies(self.context)
            
            self.page = self.context.new_page()
            
            # Если нет cookies — авторизуемся
            if not has_cookies:
                success, error = self._login(self.page)
                if not success:
                    self.browser.close()
                ...
                try:
                    self.page.wait_for_selector('[data-testid="primaryColumn"]', timeout=8000)
            ...
            return self.page, self.browser, None
            ...
            for btn in article.query_selector_all('[role="group"] button'):
                try:
                    ...
                    if "reply" in label.lower():
                        tweet["replies"] = self._extract_num(label)
                    elif "repost" in label.lower() or "retweet" in label.lower():
                        tweet["retweets"] = self._extract_num(label)
                    elif "like" in label.lower():
                        tweet["likes"] = self._extract_num(label)
                except:
                    pass
